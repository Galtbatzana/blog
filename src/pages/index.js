...
export default function Home() {
  ...
  function loadMore () {
      fetch(`https://dev.to/api/articles?username=ben&page=${page}&per_page=${pageSize}`)
      .then((response) => {
      return response.json();
    })
      .then((newArticles) => {
                ...
                {articles.map((item) => (
                  <div key={item.id} className="card bg-base-100 shadow-lg">
                    <div className="card-body bg-slate-50 text-[#181A2A]">
                      ...
                      {item.title}
                      <div className="badge bg-slate-200 text-[#4B6BFB]">{item.tag_list[0]}</div>
                      <div className="flex gap-4 items-center">
                        <Image src={item.user.profile_image} width={50} height={50} className="rounded-full"/>
